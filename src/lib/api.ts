 ...
 * Currently uses demo mode (in-memory data). When the backend is deployed,
 * change API_BASE_URL and set USE_API = true.
 * 
...
export const authApi = {
  login: async (email: string, password: string): Promise<ApiResponse<LoginResponse>> => {
    if (!USE_API) {
      // Demo mode - handled by AppContext
      return { success: false, error: 'Using demo mode' };
    }
