...
export default function ProposalDetailPage() {
  ...
  const handleVACheckSheetComplete = (checkSheet: any) => {
    ...
    const newAuditEntry = {
      ...
      action: 'VA Check Sheet completed — documents verified',
    ...
    addNotification({
      type: 'info',
      title: 'VA Check Sheet Completed',
      titleTh: 'เช็คเอกสาร VA เสร็จสิ้น',
      message: `${proposal.proposalNo || 'Draft'} "${proposal.partName}" — documents verified by VA Team`,
      messageTh: `${proposal.proposalNo || 'ร่าง'} "${proposal.partName}" — เอกสารถูกตรวจสอบแล้วโดย VA Team`,
  ...
  const handleAction = (judgement: JudgementType) => {
    ...
    // Block approval if VA Check Sheet is not completed
    if (judgement === 'approve' && !proposal?.vaCheckSheet?.isComplete) {
      alert(language === 'th' 
        ? 'ไม่สามารถอนุมัติได้ — VA PROPOSAL INFORMATION CHECK SHEET ยังไม่ถูกตรวจเช็คและอนุมัติ' 
        : 'Cannot approve — VA PROPOSAL INFORMATION CHECK SHEET has not been checked and approved yet');
      return;
    }
            ...
            {isApprover && proposal.status === 'pending' && (
              <Card className="border-primary">
                ...
                <CardContent className="space-y-4">
                  {/* VA Check Sheet Status Indicator */}
                  {proposal.vaCheckSheet?.isComplete ? (
                    <div className="flex items-center gap-2 p-2.5 rounded-lg bg-success/10 border border-success/20 text-success text-sm font-medium">
                      <CheckCircle size={16} />
                      {language === 'th' ? 'VA Check Sheet ผ่านแล้ว ✓' : 'VA Check Sheet Passed ✓'}
                      {proposal.vaCheckSheet.checkedByName && (
                        <span className="text-muted-foreground font-normal text-xs ml-auto">
                          — {language === 'th' ? 'โดย' : 'by'} {proposal.vaCheckSheet.checkedByName}
                        </span>
                      )}
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 p-2.5 rounded-lg bg-warning/10 border border-warning/20 text-warning text-sm font-medium">
                      ...
                      {language === 'th' ? 'VA Check Sheet ยังไม่ผ่าน — ไม่สามารถอนุมัติได้' : 'VA Check Sheet not yet completed — cannot approve'}
                    </div>
                  )}
